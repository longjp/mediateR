...
gR <- MakeGraphNELObject(sim_params$path,sim_params$xx_direct,
                         sim_params$mm_direct,dag$path)
attrs <- list()
attrs$edge <- list()
attrs$edge$fontsize <- 12
edgeAttrs <- MakeedgeAttrs(sim_params$path_model,sim_params$xx_direct,
                           sim_params$mm_direct,path=dag$path)
```


```{r compute_estimate,echo=FALSE,eval=TRUE}
fit <- ComputePath(dat,mmr=TRUE)
...
dat2$path[,] <- 1
fit2 <- ComputePath(dat2,mmr=TRUE)
...
for(ii in 1:B){
  ...
  fit_boot <- ComputePath(dat_boot,mmr=TRUE)
