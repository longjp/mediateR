...
## ----compute_estimate,echo=TRUE,eval=TRUE--------------------------------
fit <- ComputePath(dat)
eff_est <- ComputeEffectsLinear(fit)

## ----plot_subgraph,echo=TRUE,eval=TRUE,fig.height=6,fig.width=12---------
...
## ----merge_true_est,echo=TRUE,eval=TRUE----------------------------------
...
## ----simulation_params2,echo=TRUE,eval=TRUE------------------------------
sim_params$family <- "binomial"
dat <- SimulateData(sim_params)

## ----plot_make2,echo=TRUE,eval=TRUE--------------------------------------
gR <- mediateR:::MakeGraphNELObject(sim_params$path,sim_params$xx_direct,sim_params$mm_direct)
attrs <- list()
attrs$edge <- list()
attrs$edge$fontsize <- 12
...
## ----compute_estimate2,echo=TRUE,eval=TRUE-------------------------------
fit <- ComputePath(dat)

## ----plot_subgraph2,echo=TRUE,eval=TRUE,fig.height=6,fig.width=12--------
gR2 <- mediateR:::MakeGraphNELObject(fit$path_model,fit$xx_direct,fit$mm_direct)
attrs2 <- list()
attrs2$edge <- list()
attrs2$edge$fontsize <- 12
...
## ----xx_effects_decomp2, echo=TRUE,eval=TRUE-----------------------------
...
## ----bootstrap2,echo=TRUE,eval=TRUE--------------------------------------
B <- 100
...
## ----kable_table2,echo=TRUE----------------------------------------------
kable(eff_comb,digits=2) %>%
  kable_styling(bootstrap_options = "striped", full_width = F) %>%
  add_header_above(c(" ", "direct" = 2, "indirect" = 2, "total" = 2))

## ----bootstrap_plot2,echo=TRUE,fig.height=4.5,fig.width=4.5--------------
hist(total_xx1,main="Bootstrap Samples SNP1 Total Effect",xlab="Estimate")
...
## ----simulation_params3,echo=TRUE,eval=TRUE------------------------------
sim_params$family <- "cox"
dat <- SimulateData(sim_params)
rmean <- max(as.matrix(dat$y)[,1])

## ----plot_make3,echo=TRUE,eval=TRUE--------------------------------------
gR <- mediateR:::MakeGraphNELObject(sim_params$path,sim_params$xx_direct,sim_params$mm_direct)
attrs <- list()
attrs$edge <- list()
attrs$edge$fontsize <- 12
...
## ----compute_estimate3,echo=TRUE,eval=TRUE-------------------------------
fit <- ComputePath(dat)

## ----plot_subgraph3,echo=TRUE,eval=TRUE,fig.height=6,fig.width=12--------
gR2 <- mediateR:::MakeGraphNELObject(fit$path_model,fit$xx_direct,fit$mm_direct)
attrs2 <- list()
attrs2$edge <- list()
attrs2$edge$fontsize <- 12
...
## ----xx_effects_decomp3, echo=TRUE,eval=TRUE-----------------------------
## compute true effects by approximating integral with large sample size
...
## ----bootstrap3,echo=TRUE,eval=TRUE--------------------------------------
B <- 100
total_xx1 <- rep(0,length=B)
for(ii in 1:B){
  ix <- sample(1:sim_params$n,replace=TRUE)
...
## ----kable_table3,echo=TRUE----------------------------------------------
kable(eff_comb,digits=2) %>%
  kable_styling(bootstrap_options = "striped", full_width = F) %>%
  add_header_above(c(" ", "direct" = 2, "indirect" = 2, "total" = 2))

## ----bootstrap_plot3,echo=TRUE,fig.height=4.5,fig.width=4.5--------------
hist(total_xx1,main="Bootstrap Samples SNP1 Total Effect",xlab="Estimate")
abline(v=xx1total_sample,lwd=3,col='blue')
abline(v=xx1total_true,lwd=3,col='red')
legend("topright",c("Point Estimate","True"),col=c("blue","red"),lwd=2)
